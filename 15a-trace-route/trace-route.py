...
db = '/Users/telliott/Library/CloudStorage/Dropbox/'
r = 'Github/maps2/'

if target.startswith('I'):
    fn = db + r + 'data-interstates/' + target
elif target.startswith('U'):
    fn = db + r + 'data-highways/' + target

with open(fn + '-ends.txt') as fh:
    short_segments = fh.read().strip().split('\n\n')
    
# alter sys.path to add our utils

sys.path.insert(0, db + r + 'scripts')
import utils
